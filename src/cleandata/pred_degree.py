...
def train_model_predict_node_degree(df_pos, num_epochs=100, lr=0.01, batch_size=128, scheduler_step_size=30, scheduler_gamma=0.9, print_epoch=True, step_print=10):
    ...
    def train_function_degree_pred_FFW(df_train, df_test, drop_col, num_epochs=100, lr=0.01, batch_size=128, scheduler_step_size=30, scheduler_gamma=0.9, print_epoch=True, step_print=10):
...
def degree_pred_export_model(model, input_channel, output_channel, hidden_channel, scaler, folder_name='degree_pred_conf'):
    ...
    get the data path
    """
    folder_name = os.path.abspath(os.path.join(current_path, '../../data/degree_pred_conf'))
...
def degree_pred_import_model(folder_name='degree_pred_conf'):
    ...
    modal_path = os.path.abspath(os.path.join(folder_name, 'model.pt'))
    # load the saved PyTorch model state dictionary
    model_state_dict = torch.load(modal_path)
...
def main_pred_degree(df_data, df_pos, new_node_lat, new_node_lon, train_new = False,
    ...
    G = nx.from_pandas_edgelist(df_data, source='st_id', target='st_tg', edge_attr=['train_max_speed', 'distance', 'travel_time'])
    ...
    G_degree = pd.DataFrame(G.degree(G.nodes()), columns = ['node', 'degree']).set_index('node')

    # obtain df_pos2 which have information of state_num and degree
    df_pos2 = df_pos.set_index('node')
