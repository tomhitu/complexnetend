...
def prep_classify_data(df_pos, df_new_node, split_portion=0.1):
    ...
    X_train, X_test, y_train, y_test = train_test_split(df_pos[['lat', 'lon']], df_pos['state_num'], test_size=split_portion, random_state=42)
    ...
    df_new_node.loc[:, 'state_num'] = rf.predict(df_new_node[['lat', 'lon']])
...
def edge_pred_export_model(model, input_channel, output_channel, hidden_channel, scaler, folder_name='edge_pred_conf'):
    ...
    get the data path
    """
    folder_name = os.path.abspath(os.path.join(current_path, '../../data/edge_pred_conf'))
    ...
    jsondata_path = os.path.abspath(os.path.join(folder_name, 'channels.json'))

    with open(jsondata_path, 'w') as f:
        json.dump(data, f)
...
def map_pos_into_data(df_data, df_pos):
    ...
    df_data = pd.merge(df_data, df_pos, left_on='st_id', right_on='node', how='left')
    df_data = pd.merge(df_data, df_pos, left_on='st_tg', right_on='node', how='left', suffixes=('_id', '_tg'))

    # drop the 'node' columns
    df_data.drop(['node_id', 'node_tg'], axis=1, inplace=True)
    
    return df_data

''' =================================================================================================
function reverse train speed
================================================================================================= '''
def reverse_train_speed(speed):
    if speed >= 320:
        return 'Gxxx', 320  # may be G or C
    elif speed >= 260:
        return 'Dxxx', 260
    elif speed >= 160:
        return 'Zxxx', 160 # may be Z or T
    elif speed >= 120:
        return 'Kxxx', 120
    else:
        return 'Xxxx', 100
...
================================================================================================= '''
def plot_all_on_map(G, node_attrs, high_light_node1=None, high_light_node2=None):
    # Extract latitude and longitude from the node attributes
    pos = {node_id: (node_attrs[node_id]['lon'], node_attrs[node_id]['lat']) for node_id in G.nodes}
    ...
    # Draw all non-highlighted nodes
    non_highlighted_nodes = [node_id for node_id in G.nodes if node_id not in high_light_node1 + high_light_node2]
    nx.draw_networkx_nodes(G, pos, nodelist=non_highlighted_nodes, node_size=0.5, node_color='gray', ax=ax)
    nx.draw_networkx_edges(G, pos, width=0.2, edge_color='black', ax=ax)

    # Draw the highlighted nodes on top
    ...
    if high_light_node2 is not None:
        ...
        nx.draw_networkx_nodes(G, pos, nodelist=highlighted_nodes2, node_size=1, node_color='blue', ax=ax)
    ...
    if high_light_node1 is not None and high_light_node2 is not None:
        ...
        nx.draw_networkx_edges(G, pos, edgelist=red_edges, width=0.2, edge_color='red', ax=ax)
...
def main_edge_feature_pred(df_data, df_pos, new_node_lat, new_node_lon, n_degree, train_new = False,
                           ...
                           scheduler_step_size=10, scheduler_gamma=0.8, print_epoch=True, step_print=10,
                           folder_name = 'edge_pred_conf'
                          ):
    # map lat-lon to df
    df_data2 = map_pos_into_data(df_data, df_pos)
    df_data2 = df_data2.drop(['st_id', 'st_tg'], axis=1) 
    ...
    new_pair = df_pos[df_pos['node'].isin(neighbor_node)].copy()
    new_pair.loc[:, 'lat_id'] = df_new_node.loc[0, 'lat']
    new_pair.loc[:, 'lon_id'] = df_new_node.loc[0, 'lon']
    new_pair.loc[:, 'state_num_id'] = df_new_node.loc[0, 'state_num']
    new_pair.loc[:, 'lat_tg'] = new_pair.loc[:, 'lat']
    new_pair.loc[:, 'lon_tg'] = new_pair.loc[:, 'lon']
    new_pair.loc[:, 'state_num_tg'] = new_pair.loc[:, 'state_num']
    new_pair_node = new_pair['node']
    new_pair = new_pair.drop(['node', 'lat', 'lon', 'state_num'], axis=1)
    ...
    max_st_id = max(df_pos['node'])
    new_st_id = max_st_id+1

    # get new df edge feature
    df_edge_pred = pd.DataFrame(edge_pred, columns=['train_max_speed', 'distance', 'travel_time'])
    df_edge_pred['st_tg'] = neighbor_node
    df_edge_pred['st_id'] = new_st_id
    ...
    df_new_node2['node'] = max(df_data['st_id'])+1
    df_pos2 = pd.concat([df_pos, df_new_node2], ignore_index=True)
    df_pos2[df_pos2['node']==max(df_data['st_id'])+1]
